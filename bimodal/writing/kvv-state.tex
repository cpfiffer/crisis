...
I highlight the historical context of "price as a random variable"  to illustrate my solution method. I seek to describe the properties of the joint density $P(p, f, x, s, \eta_j)$ so as to reduce the problem of finding a price that rationalizes a NREE.

\begin{itemize}
    \item $p \mid f, x, s$ is non-stochastic.
    \item $p$ with no conditioning must be of the same family as $f$.
    \item $p$ is noisier than $f$ due to the stochastic supply $x$. I.e., $\Var[p] > \Var[f]$.
    \item $P(p,f,x,s,\eta_j) = P(f,x,s,\eta_j) = P(p)$
\end{itemize}
